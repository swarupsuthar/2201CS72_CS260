...
-- 3. Write a SQL query to find employees who are managers.
SELECT *
FROM employees
WHERE emp_id IN (SELECT manager_id FROM departments WHERE manager_id IS NOT NULL);
...
-- 5. Write a SQL query to join employees with their respective departments.
...
-- 7. Write a SQL query to find employees who are not managers.
SELECT *
FROM employees
...
-- 9. Write a SQL query to project the department names and locations from departments table.
...
-- 11. Write a SQL query to find employees who are managers in the 'Sales' department.
SELECT *
FROM employees
WHERE department_id = (SELECT department_id FROM departments WHERE department_name = 'Sales') AND manager_id IS NOT NULL;
...
-- 13. Write a SQL query to find employees who are not assigned to any projects.
SELECT *
FROM employees
...
JOIN projects p ON en.project_id = p.project_id;

-- 15. Write a SQL query to find employees whose salaries are not within the range ?50000 to ?70000.
SELECT *
FROM employees
